...
#Định nghĩa toán tử Prewitt (row)
PreRow = np.array(([-1,-2,-1],
                        [0, 0, 0],
                        [1, 2, 1],
                       ), dtype="float")
#Định nghĩa toán tử Prewitt (col)
PreCol = np.array(([1,0,-1],
                        [2,0, -2],
                        [1, 0, -1],
                       ), dtype="float")
...
def loc_min(img):
    ...
    for i in range(1, m - 1):
        for j in range(1, n - 1):
            ...
            temp = min(temp)     # nếu lọc min thì thay hàm max bằng min(temp)
            img_new[i, j] = temp
    return img_new

def SobelXY(im, col, row):
    r = Tich_chap(im, row)
    c = Tich_chap(im, col)
    P = r + c
    return r
...
image = cv2.imread('noisy_image.tif', 0)
...
imgmin_3x3 = loc_min(image) #Gọi hàm lọc
ax7.imshow(imgmin_3x3, cmap='gray')
ax7.set_title("Ảnh lọc min 3x3")

p = SobelXY(image,PreCol, PreRow) #Gọi hàm lọc
print(p)
ax8.imshow(p, cmap='gray')
ax8.set_title("SobelXY mask")
